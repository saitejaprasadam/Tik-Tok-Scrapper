...
class Api():
    ...
    def home_list(self, user_data={}):
        ...
        if (user_data.__len__() > 0):
            data = self.helper.request_get(url, session=self.active_user['cookies'])
        else:
    ...
    def user_video_list(self, user_id='6594722549190574086', cursor=0, session={}):
        url = self.api_url + "aweme/v1/aweme/post/?user_id=" + str(
            user_id) + "&max_cursor=" + str(cursor) + "&type=0&offset=100&count=20&pull_type=1&" + self.helper.query(
            self.helper.default_variable(self.global_variable))
        if (session.__len__() > 0):
            data = self.helper.request_get(url, session=session)
        else:
